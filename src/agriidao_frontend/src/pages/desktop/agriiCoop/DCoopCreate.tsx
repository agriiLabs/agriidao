...
const DCoopCreate = () => {
  ...
  const [coopName, setCoopName] = useState<File | null>(null);
  ...
  const schema = z.object({
    ...
    payoutFrequency: z.number().min(1, { message: "Payout frequency required" }),
    ...
    managementFee: z.number().min(0, { message: "Management fee required" }),
  });
  ...
  const handleSave = async (data: FormData) => {
    ...
    // const unitImage = await uploadAsset();
    try {
        let body: CoopRequest = {
          ...
          ticker: data.ticker,
          managementFee: BigInt(data.managementFee) / BigInt(100),
          unitImage: [],
        };
    
        dispatch(setCoopRequest(body));
        navigate("/d/coop-create/preview");  
      } catch (error) {
  ...
  //     const file_path = `coops/${coopName?.name}`;
  //     try {
  //       if (!coopName) {
  //         throw new Error("No file selected for upload");
  //       }
  //       const res = await uploadFile(coopName, file_path);
  //       console.error("Asset uploaded successfully", res);
  //       return res;
  //     } catch (error) {
